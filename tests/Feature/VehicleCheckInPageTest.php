...
class VehicleCheckInPageTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_manager_can_view_vehicle_check_in_page(): void
    {
        $user = $this->makeUserWithRole(RoleSlug::Manager);

        $response = $this->actingAs($user)->get(route('vehicles.check-in'));

        $response->assertOk();
        $response->assertSee('Vehicle Check-In');
    }
