<x-layouts.app>
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <x-ui.stat-card title="Jobs Completed" :value="$metric->jobs_completed ?? 0" />
        <x-ui.stat-card title="Revenue Generated" :value="number_format($metric->revenue_generated ?? 0, 2)" />
        <x-ui.stat-card title="Average Rating" :value="$metric->average_rating ? number_format($metric->average_rating, 1) : '—'" />
        <x-ui.stat-card title="Employee" :value="$metric->employee?->full_name ?? '—'" />
    </div>
    ...
    <x-ui.card>
        <dl class="space-y-3 text-sm">
            ...
            <div class="flex justify-between gap-4"><dt class="text-slate-500">Average Rating</dt><dd>{{ $metric->average_rating ? number_format($metric->average_rating, 1) : '—' }}</dd></div>
        </dl>
    </x-ui.card>
</x-layouts.app>
