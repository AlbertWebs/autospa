    ...
    >
        ...
        @foreach ($invoices as $invoice)
            <tr class="asp-table-row">
                <x-ui.td primary mono>{{ $invoice->number ?? '#'.$invoice->id }}</x-ui.td>
                <x-ui.td>{{ $invoice->customer?->full_name ?? '—' }}</x-ui.td>
                <x-ui.td>{{ number_format($invoice->total ?? 0, 2) }}</x-ui.td>
                <x-ui.td>
                    <x-ui.badge color="indigo">{{ $invoice->status ?? 'pending' }}</x-ui.badge>
                </x-ui.td>
                <x-ui.td align="right">
