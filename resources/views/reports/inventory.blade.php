<x-layouts.app>
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <x-ui.stat-card label="Low Stock" :value="$report['low_stock'] ?? 0" />
        <x-ui.stat-card label="Stock Value" :value="number_format($report['stock_value'] ?? 0, 2)" />
    </div>
    <x-ui.card>
            ...
            @foreach ($report as $key => $value)
                @if(is_scalar($value))
                    <div class="flex justify-between gap-4 rounded-lg bg-slate-50 px-4 py-3 dark:bg-slate-800">
                        ...
                        <dd class="font-medium">{{ is_numeric($value) && str_contains($key, 'value') ? number_format($value, 2) : $value }}</dd>
                    </div>
                @endif
            @endforeach
        </dl>
