...
<x-ui.index-page
    eyebrow="Operations"
    title="Bookings"
    subtitle="Walk-ins, pending, completed, and cancelled bookings in one list."
...
>
    <form method="GET" action="{{ route('bookings.index') }}" class="mb-6 rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-brand-border/60 dark:bg-brand-surface-high">
        <div class="grid gap-4 md:grid-cols-4">
            <x-ui.form-field label="Date" for="booking_date" hint="Filter by scheduled date.">
    ...
    <div class="mb-6 flex flex-wrap gap-2">
        <a href="{{ route('bookings.index', ['type' => BookingType::WalkIn->value]) }}" class="rounded-xl border border-slate-200/80 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:border-brand-primary/40 hover:text-brand-primary-dim dark:border-brand-border/60 dark:bg-brand-surface-high dark:text-slate-200">Walk-ins</a>
        <a href="{{ route('bookings.index', ['status' => BookingStatus::Pending->value]) }}" class="rounded-xl border border-slate-200/80 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:border-brand-primary/40 hover:text-brand-primary-dim dark:border-brand-border/60 dark:bg-brand-surface-high dark:text-slate-200">Pending</a>
        <a href="{{ route('bookings.index', ['status' => BookingStatus::Completed->value]) }}" class="rounded-xl border border-slate-200/80 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:border-brand-primary/40 hover:text-brand-primary-dim dark:border-brand-border/60 dark:bg-brand-surface-high dark:text-slate-200">Completed</a>
        <a href="{{ route('bookings.index', ['status' => BookingStatus::Cancelled->value]) }}" class="rounded-xl border border-slate-200/80 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:border-brand-primary/40 hover:text-brand-primary-dim dark:border-brand-border/60 dark:bg-brand-surface-high dark:text-slate-200">Cancelled</a>
    </div>
    ...
    <x-ui.data-table
        ...
        empty-description="Try adjusting the date or status filters."
    >
        <x-slot name="header">
