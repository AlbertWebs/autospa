...
// Show the navigation progress bar quickly so page loads feel responsive.
Turbo.setProgressBarDelay(100);

// Loading state for any submit button while its form is being processed.
document.addEventListener('turbo:submit-start', (event) => {
    const submitter = event.detail.formSubmission?.submitter;

    if (submitter) {
        submitter.classList.add('asp-btn-loading');
...
document.addEventListener('turbo:submit-end', (event) => {
    ...
    if (submitter) {
        submitter.classList.remove('asp-btn-loading');
