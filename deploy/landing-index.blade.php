@php
    ...
    // Descriptions can contain internal pricing config (commission rates,
    // revenue targets) that must never surface on the public site.
    $publicDescription = function ($description): string {
        $sentences = preg_split('/(?<=[.!?])\s+/', trim((string) $description)) ?: [];

        return trim(implode(' ', array_filter(
            $sentences,
            fn ($sentence) => ! preg_match('/commission|target revenue/i', $sentence)
        )));
    };
@endphp
    ...
    <div class="landing">
        ...
        <section class="landing-hero" aria-label="Welcome">
            <div class="landing-hero-media" aria-hidden="true">
                <link rel="preconnect" href="https://expresscarwash-media.s3.eu-north-1.amazonaws.com" crossorigin>
                {{-- Poster shows instantly; the video src is attached after page
                     load so the 7 MB file never competes with critical assets. --}}
                <video
                    ...
                    playsinline
                    preload="none"
                    poster="https://expresscarwash-media.s3.eu-north-1.amazonaws.com/video/bg-poster.jpg"
                    data-hero-video="https://expresscarwash-media.s3.eu-north-1.amazonaws.com/video/bg-720.mp4"
                ></video>
                <script>
                    (function () {
                    ...
                    <div class="landing-menu">
                        ...
                        <ul class="landing-menu-list">
                            @foreach ($categoryServices as $service)
                                @php $serviceDescription = $publicDescription($service->description); @endphp
                                <li class="landing-menu-item">
                                    <div class="landing-menu-copy">
                                        ...
                                        @if (filled($serviceDescription))
                                            <p class="landing-menu-desc">{{ $serviceDescription }}</p>
                                        @endif
                                    </div>
                                    <div class="landing-menu-meta">
                                        @if ($service->duration_minutes)
                                            <span class="landing-menu-duration">
                                                <x-landing.icon name="clock" />
                ...
                <div class="landing-visit-grid">
                    <div class="landing-visit-block">
                        <ul class="landing-contact-list">
                            @if (filled($address))
                                <li>
                                    <span class="landing-contact-icon"><x-landing.icon name="pin" /></span>
                                    <div>
                            ...
                            @if (filled($phone))
                                <li>
                                    <span class="landing-contact-icon"><x-landing.icon name="phone" /></span>
                                    <div>
                            ...
                            @if (filled($email))
                                <li>
                                    <span class="landing-contact-icon"><x-landing.icon name="mail" /></span>
                                    <div>
                    ...
                    <div class="landing-visit-block">
                        <p class="landing-meta-label landing-meta-label-icon">
                            <x-landing.icon name="clock" />
                            Hours
                        </p>
                        @if ($hours->isNotEmpty())
            ...
            <div class="landing-shell landing-book">
                ...
                <form method="POST" action="{{ route('landing.book') }}" class="landing-form">
                    ...
                    <div class="landing-form-grid">
                        ...
                        <label class="landing-field landing-field-wide">
                            ...
                            <input type="datetime-local" name="scheduled_at" value="{{ old('scheduled_at') }}" min="{{ now()->format('Y-m-d\TH:i') }}" required>
                        </label>
                    </div>
                    ...
                    <p class="landing-total" data-booking-total hidden>
                ...
                <script>
                    (function () {
                        ...
                        var totalWrap = form.querySelector('[data-booking-total]');
                        var totalValue = totalWrap ? totalWrap.querySelector('strong') : null;
                        var boxes = form.querySelectorAll('input[name="service_ids[]"]');

                        var updateTotal = function () {
                            var total = 0;
                            boxes.forEach(function (box) {
                                if (box.checked) total += parseFloat(box.dataset.price || '0');
                            });
                            if (totalWrap) {
                                totalWrap.hidden = total <= 0;
                                if (totalValue) {
                                    totalValue.textContent = 'KES ' + Math.round(total).toLocaleString('en-KE');
                                }
                            }
                        };
            ...
            <div class="landing-shell">
                <div class="landing-footer-inner">
                    <div class="landing-footer-about">
                        ...
                        <p class="landing-footer-tag">
                            Auto Spa in {{ $locality }} — exterior wash, interior detailing, and carpet care with on-time slots.
                        </p>
                    </div>
                    <nav class="landing-footer-col" aria-label="Footer">
                        <p class="landing-footer-heading">Explore</p>
                        <a href="#services">Services</a>
                        <a href="#visit">Visit us</a>
                        <a href="#book">Book a slot</a>
                    </nav>
                    <div class="landing-footer-col">
                        <p class="landing-footer-heading">Contact</p>
                        @if ($telUrl)
                            <a href="{{ $telUrl }}">{{ $phone }}</a>
                        @endif
                        @if ($whatsappUrl)
